...
Designed specifically for London City Airport (LCY) operational constraints:
- Single runway (09/27): 1,508 metres long
- 19 aircraft stands (gates)
- Mixed operations (arrivals and departures on same runway)
- Priority system: arrivals take precedence over departures
- Realistic taxi times with stochastic variation

Coordinate System:
- x: Runway direction (East, 0 to 1508 m)
- y: Lateral (North positive)
- z: Altitude (not used in ground ops)
"""
...
def generate_gate_positions(num_gates: int = 19, runway_length: float = RUNWAY_LENGTH) -> np.ndarray:
    ...
    alternating between north (+y) and south (-y) sides.
    This layout minimises taxi distances and conflicts.
    
    Parameters
    ----------
    num_gates : int
        Number of gates to generate (default 19 for London City Airport)
    runway_length : float
        Length of runway in metres (default 1508 m)
        
    Returns
    -------
    np.ndarray
        Array of shape (num_gates, 3) with gate positions [x, y, z] in metres
    """res from runway centreline
    ...
    for i in range(num_gates):
        ...
        # Position along runway (staggered to reduce congestion)
        x_position = gate_spacing * (i // 2 + 1)
        y_position = side * lateral_offset
        z_position = 0.0  # Ground level runway (staggered)
        x_position = gate_spacing * (i // 2 + 1)
        y_position = side * lateral_offset
        z_position = 0.0
...
class GroundOperations:
        ...
        Sets up runway management, gate allocation, and queue systems.
        London City Airport operates with a single runway serving both
        arrivals and departures, requiring careful scheduling.

        Parameters
        ----------
        runway_length : float
            Length of the runway in metres (1,508 m for LCY).
        num_gates : int
            Total number of aircraft stands available (19 for LCY).
        runway_slope : float
            Runway slope angle (theta) in radians (typically ~0.01 rad or 0.6\u00b0).

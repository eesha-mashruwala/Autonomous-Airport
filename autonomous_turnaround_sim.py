...
INTER_ARRIVAL_MEAN = 50.0  # seconds between new arrivals (mean)
...
class SingleAircraftTurnaround:
    ...
    def _log_intro(self):
        if not self.feed.banner_printed:
            print("\n" + "=" * 70)
            print("AUTONOMOUS AIRPORT SYSTEM SIMULATION")
            print("=" * 70)
    ...
    def step(self, global_time: float):
        ...
        # Free runway when its busy window expires (for arrivals/vacating only)
        if getattr(self.ground_ops, "runway_busy", False) and self.sim_time >= self.ground_ops.runway_free_time:
            # Only auto-release for non-departure states (arrivals vacating runway)
            if self.state in ("vacating_runway", "gate_assignment"):
                self.ground_ops.release_runway()
                self.feed.runway_status(False, "vacate timer elapsed")
                self._log("Runway now FREE (vacate timer elapsed)")
    ...
    def _runway_queue(self):
        ...
        # CRITICAL: Check runway_busy flag directly, not the time-based check
        # This prevents race conditions where multiple aircraft see runway as available
        if self.ground_ops.runway_busy:
            if not self.runway_hold_depart_logged:
        ...
        # Runway free -> IMMEDIATELY claim it before starting departure sim
        # This prevents race conditions with other waiting aircraft
        safety_duration = 600.0
        self.ground_ops.mark_runway_busy(self.plane, safety_duration, self.sim_time)
        
        # Now start departure sim
        ...
        self.info_window.add_line("Runway blocked until airborne (120m altitude)")
        self.feed.announce_clearance(f"{self.plane.id} cleared for takeoff")
        self.feed.runway_status(True, f"{self.plane.id} departing")
        self._log(
            f"Cleared for takeoff. Runway will be released when aircraft reaches 120m altitude"
        )
        return self.position
...
class EmergencyArrivalSimulator(ArrivalSimulator):
    ...
    def set_hold(self, active: bool, estimated_hold_time: float = 60.0):
        ...
        if active:
            # Use a fixed, reasonable holding radius (standard holding pattern)
            # Typical holding patterns are 1-2 NM (1850-3700m) but we'll use smaller for visibility
            self.hold_target_speed = CRUISE_SPEED
            self.estimated_hold_time = max(15.0, estimated_hold_time)
            self.hold_radius = 400.0  # Fixed 400m radius for compact, visible holding patterns
    ...
    def step(self):
        ...
        if not self.emergency_mode:
            if self.global_time >= self.next_spawn_time and not self.emergency_active:
                ...
                if spawn_emergency:
                    self.feed.status_update("Emergency aircraft detected in airspace")
                    self._spawn_emergency_flight()
...
def run_visual_sim(max_frames: int = 6000, show: bool = True, emergency_mode: bool = False, emergency_chance: float = 1.0 / 10000.0):
    ...
    # # Load and display x-y plane background image
    # try:
    #     img = mpimg.imread('x-y-plane.jpg')
    #     x_range = np.linspace(-AIRSPACE_RADIUS, AIRSPACE_RADIUS, img.shape[1])
    #     y_range = np.linspace(-AIRSPACE_RADIUS, AIRSPACE_RADIUS, img.shape[0])
    #     X, Y = np.meshgrid(x_range, y_range)
    #     Z = np.full_like(X, -5.0)
    #     ax.plot_surface(X, Y, Z, facecolors=img/255.0, rstride=5, cstride=5, 
    #                    shade=False, zorder=0, alpha=0.9)
    # except Exception as e:
    #     print(f"Warning: Could not load x-y-plane.jpg: {e}")

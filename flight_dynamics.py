...
Implements the physics of aircraft flight through various phases using
numerical integration of ordinary differential equations (ODEs).

Flight Phases:
- Ground roll (takeoff): 1D acceleration along runway
- Climb: 3D kinematics with increasing altitude
- Cruise/Holding: Constant altitude circular or straight flight
- Approach: Controlled descent at fixed glide angle
- Landing: Ground roll deceleration with brakes and thrust reversers

Integration Method:
- Euler method with configurable time steps
- Records telemetry for post-flight analysis
- Handles edge cases (stall, negative velocity, etc.)
...
class WindModel:
    ...
    Simulates realistic atmospheric wind conditions with:
    - Slowly varying base wind (speed and direction)
    - Periodic random walk adjustments (every 25-60 seconds)
    - Continuous turbulent gusts (Gaussian noise)
    - Small vertical wind component (updrafts/downdrafts)
    
    Wind affects aircraft groundspeed but not indicated airspeed.
    In this simplified model, wind is applied as a velocity offset.
    
    Physical Realism:
    - Wind direction changes gradually (max 8° per update)
    - Speed varies within specified bounds
    - Gust intensity controlled by standard deviation
    - Smoothing parameter prevents abrupt changes
    """

    def __init__(
        self,
        min_speed: float = 0.0,          # m/s - Minimum wind speed (calm conditions)
        max_speed: float = 11.0,         # m/s - Maximum wind speed (~21 knots, moderate breeze)
        change_interval: Tuple[float, float] = (25.0, 60.0),  # seconds between target updates
        max_dir_step_deg: float = 8.0,   # degrees - Maximum direction change per update
        max_speed_step: float = 2.0,     # m/s - Maximum speed change per update
        smoothing: float = 6.0,          # seconds - Exponential smoothing time constant
        gust_sigma: float = 0.5,         # m/s - Standard deviation of turbulent gusts
        vertical_sigma: float = 0.15,    # m/s - Standard deviation of vertical component
    ):
        # Wind speed bounds (typical range for safe flight operations)
        ...
        self.smoothing = max(1.0, smoothing)  # Ensure positive smoothing
        
        # Turbulence parameters
        self.gust_sigma = gust_sigma          # Horizontal gusts
        self.vertical_sigma = vertical_sigma  # Vertical air movement

        # Initialise wind state with random starting conditions
        self.direction = random.uniform(0.0, 2 * math.pi)  # radians, 0=East, π/2=North
        ...
        # Current wind vector [vx, vy, vz] in m/s
        self.vector = np.zeros(3)

    def update(self, dt: float) -> np.ndarray:
        """
        Advance the wind state and return the current wind vector.
        
        Uses a random walk with exponential smoothing to simulate
        realistic atmospheric wind evolution.
        ...
        # Exponential smoothing: gradually approach target
        ...
        # Continuous turbulence: small jitter to avoid perfectly smooth flow
        jitter_dir = math.radians(random.uniform(-0.2, 0.2))
        self.direction = _wrap_angle(self.direction + jitter_dir * dt)
        
        # Add Gaussian gust noise
        gust = random.gauss(0.0, self.gust_sigma)
        effective_speed = max(0.0, self.speed + gust)

        # Compute horizontal wind components
        vx = effective_speed * math.cos(self.direction)  # Eastward
        vy = effective_speed * math.sin(self.direction)  # Northward
        vz = random.gauss(0.0, self.vertical_sigma)      # Vertical (updraft/downdraft)
...
class FlightDynamics:
    ...
    def __init__(self, plane: Plane):
        ...
        Creates a flight dynamics simulator for the specified aircraft.
        Maintains complete state history via telemetry recording.

        Parameters
        ----------
        plane : Plane
            The physical plane object containing geometry, mass, and aerodynamic properties.
        """
        self._plane = plane  # Protected reference to the aircraft
        self._telemetry: List[Dict] = []  # Stores complete flight history

        # Flight state variables (integrating ODEs updates these each timestep)
        self.t = 0.0        # Time (s) - simulation time
        self.x = 0.0        # Position x (m) - eastward displacement
        self.y = 0.0        # Position y (m) - northward displacement
        self.z = 0.0        # Altitude (m) - positive upwards
        self.V = 0.0        # True airspeed (m/s) - velocity magnitude
        self.gamma = 0.0    # Flight path angle (radians) - positive=climbing, negative=descending
        self.psi = 0.0      # Heading angle (radians) - 0=East, π/2=North
        self.phi = 0.0      # Bank angle (radians) - positive=right bank

        # Current forces (newtons) - recomputed each timestep
        self.T = 0.0        # Thrust (N) - engine force
        self.D = 0.0        # Drag (N) - aerodynamic resistance
        self.L = 0.0        # Lift (N) - aerodynamic vertical force
        self.F_rr = 0.0     # Rolling friction (N) - tyre-runway interaction
        self.F_brake = 0.0  # Braking force (N) - wheel brakes

        # Control inputs (normalised 0-1)
        self.throttle_cmd = 0.0  # 0=idle, 1=full throttle
        self.brake_cmd = 0.0     # 0=released, 1=maximum braking

        # Runway parameters
        self._theta = 0.0   # Runway slope angle (radians) - typically small (~0.01 rad)
